...
export default function StudyScreen({ deckId, onBack }: StudyScreenProps) {
  ...
  const [isReverse, setIsReverse] = useState(false); // Обратная викторина
  ...
  useEffect(() => {
    ...
    // Проверяем, это временная колода ошибок?
    ...
    if (deckToStudy) {
      setDeck(deckToStudy);
      // Берем все карточки, кроме выученных (если это не режим ошибок)
  ...
  // Определяем, какую сторону показывать как "вопрос"
    ...
    <div className="min-h-[100dvh] bg-purple-50 flex flex-col">
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className={`flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-semibold transition-colors ${
                isReverse ? 'bg-orange-500 text-white' : 'bg-white/20 text-white'
              }`}
            >
      ...
      <main className="flex-1 max-w-md mx-auto w-full px-5 py-6 flex flex-col items-center justify-center gap-6">
        ...
        >
          ...
          {/* Дополнительные стороны (Insane режим) */}
